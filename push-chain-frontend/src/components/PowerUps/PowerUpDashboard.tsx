...
const PowerUpDashboard: FC<PowerUpDashboardProps> = ({ walletAddress }) => {
  ...
  const fetchSummary = useCallback(async (address: string) => {
    setIsLoading(true);
    setError(null);
    try {
    ...
    async (event: FormEvent<HTMLFormElement>) => {
      ...
      try {
        ...
        const expiresAtSeconds = Math.floor(
          Date.now() / 1000 + durationHours * 3600
        );
        const txHash = await delegateBoost(
          renter,
          tokenId,
          amount,
          expiresAtSeconds
    ...
    return (
      <div className="powerup-card">
        ...
        <div className="powerup-grid">
          {POWER_UP_IDS.map((id) => {
            const meta =
              POWER_UP_METADATA[id as keyof typeof POWER_UP_METADATA];
            const balance = summary.balances?.[id] ?? 0;
            const locked = summary.locked?.[id] ?? 0;
            ...
            return (
              <div key={id} className="powerup-tile">
                <div className="">
                  ...
                  <div className="powerup-copy" style={{margin : "8px 0px"}}>
                    <h4>{meta.name}</h4>
      ...
      <div className="powerup-card">
        ...
        {hasCrate ? (
          <div className="crate-section">
            ...
            >
              {isOpeningCrate || isConfirmingCrate
                ? "Opening crate..."
                : "Open crate"}
            </button>
            {openCrateSuccess && openCrateHash && (
              <small>
                Tx hash: <code>{openCrateHash}</code>
              </small>
            )}
          </div>
        ) : (
      ...
      <div className="powerup-card">
        ...
        <form className="delegation-form" onSubmit={handleDelegationSubmit}>
            ...
            <label>
              ...
              <select
                ...
                onChange={(event) =>
                  setFormState((prev) => ({
                    ...prev,
                    tokenId: Number(event.target.value),
                  }))
                }
              >
                {POWER_UP_IDS.map((id) => {
          ...
          {delegationSuccess && delegateHash && (
            <small>
              Tx hash: <code>{delegateHash}</code>
            </small>
          )}
        </form>
            ...
            {ownerDelegations.length === 0 ? (
              ...
              <ul>
                {ownerDelegations.map((record) => {
                  const meta =
                    POWER_UP_METADATA[
                      record.tokenId as keyof typeof POWER_UP_METADATA
                    ];
                  return (
                    <li
                      key={`${record.ownerWallet}-${record.renterWallet}-${record.tokenId}`}
                    >
                      <div>
                      ...
                      <div className="delegation-actions">
                        <small>
                          Renter: {record.renterWallet.slice(0, 6)}…
                          {record.renterWallet.slice(-4)}
                        </small>
                        <button
                          type="button"
                ...
                {borrowerDelegations.map((record) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <small>
                        Lender: {record.ownerWallet.slice(0, 6)}…
                        {record.ownerWallet.slice(-4)}
                      </small>
                    </li>
                  );
                })}
        ...
        {cancelSuccess && cancelDelegationHash && (
          <small>
            Cancel tx: <code>{cancelDelegationHash}</code>
          </small>
        )}
      </div>
    );
    ...
    <div className="powerup-page">
      ...
      {!summary.ready && (
        <div className="powerup-warning">
          Power-up services are not initialized yet. You can still browse your
          inventory, but rewards will sync once the backend signer is
          configured.
        </div>
      )}
